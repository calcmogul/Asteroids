...
int main() {
  ...
  ProgressBar healthBar(sf::Vector2f(100.f, 19.f), "Health",
                        sf::Color(120, 0, 0), sf::Color(40, 40, 40),
                        sf::Color(50, 50, 50));
  ...
  while (mainWin.isOpen()) {
    ...
    if (!isPaused) {
      ...
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Space) &&
          shootClock.getElapsedTime().asMilliseconds() > 100) {
        // Bullet::add( myShip , mainWin );
        Planet* tempPlanet = Planet::add(
            sf::Vector2f(
                myShip.drawShape->getPosition().x +
                    1.5f * std::cos(myShip.body->GetAngle() + b2_pi / 2.f),
                myShip.drawShape->getPosition().y +
                    1.5f * std::sin(myShip.body->GetAngle() + b2_pi / 2.f)),
            (std::rand() % 49 + 1.f) / 30.f,
            sf::Color(std::rand() % 256, std::rand() % 256, std::rand() % 256));
        tempPlanet->body->SetLinearVelocity(
            b2Vec2(15.f * std::cos(myShip.body->GetAngle() + b2_pi / 2.f),
                   15.f * std::sin(myShip.body->GetAngle() + b2_pi / 2.f)));
        shootClock.restart();
      }
      ...
      // healthBar.shader.setParameter( "currentPos" , healthBar.getPosition().x
      // - myShip.shape.getPosition().x + mainWin.getSize().x / 2.f ,
      // myShip.shape.getPosition().y - healthBar.getPosition().y +
      // mainWin.getSize().y / 2.f );
      healthBar.shader.setUniform("currentPos",
                                  sf::Vector2f{6.f, mainWin.getSize().y - 6.f});
    }

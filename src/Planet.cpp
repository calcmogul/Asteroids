...
void Planet::drawAll( const Ship& ship , sf::RenderTarget& target , [[maybe_unused]] sf::RenderStates states ) {
    for ( unsigned int index = 0 ; index < m_planets.size() ; index++ ) {
        // Realign shading with position of given ship's current position
        m_planets[index]->shader.setUniform( "currentPos" , sf::Vector2f{m_planets[index]->shape.getPosition().x - ship.shape.getPosition().x + target.getSize().x / 2.f , ship.shape.getPosition().y - m_planets[index]->shape.getPosition().y + target.getSize().y / 2.f + 2.f/* offset for shading */} );
...
Planet* Planet::add( const sf::Vector2f& position , const float& radius , const sf::Color& color ) {
...
void Planet::applyUnivGravity() {
    ...
    for ( b2Body* startBody = Box2DBase::world.GetBodyList() ; startBody != nullptr ; startBody = startBody->GetNext() ) {
        for ( b2Body* moveBody = startBody->GetNext() ; moveBody != nullptr ; moveBody = moveBody->GetNext() ) {
...
Planet::Planet( const sf::Vector2f& position , const float& radius , const sf::Color& color ) : Box2DBase( &shape , position , b2_dynamicBody ) , shape( radius * 30.f ) {
    ...
    // Create the shader
    if ( !shader.loadFromFile( "resources/circleShade.frag" , sf::Shader::Type::Fragment ) ) {
        exit( 1 );
    }
